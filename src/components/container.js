...
function ContainerTitle(props) {
    return (
        <h2 className="font-semibold text-3xl text-center md:text-left">{props.title}</h2>
    );
}
...
function Container(props) {
    ...
    return (
        <div className="rounded-md bg-white m-4 p-4 shadow-md overflow-auto">
            <div className="border-b-2 border-gray-100 pb-2 flex flex-row md:justify-between justify-center">
...
function LightContainer({ children }) {
    return (
        <div className='p-4 rounded-lg flex flex-col items-center w-full'>
            {children}
        </div>
    )
}
export { Container, ContainerSection, Field, FieldAsColumn, TokenField, fromWei, LightContainer }